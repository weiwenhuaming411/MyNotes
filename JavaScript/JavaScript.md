...
<!-- 
    元素属性：
        ...
        element.(attr、text、comment).nodeValue  返回元素节点的value值  // 'heelo world'
        兄弟元素：
            element.previousSibling  返回元素紧接之前的节点(包含文本节点或注释节点)  // #text  注: 空格文本是文本节点
            element.nextSibling  返回元素紧跟的下一个节点(包含文本节点或注释节点)  // #text  注: 空格文本是文本节点
        子元素：
